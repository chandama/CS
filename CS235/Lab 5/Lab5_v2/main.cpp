...
void parse_instruction(std::string temp, std::ofstream &ofs, Pathfinder* aptr) {
	...
	if (command == "importMaze") { // command to import a new maze from a file
		...
		for (int i = 0; i < pathVec.size(); i++) { // get a string representation of the path returned
			ss << pathVec[i] << std::endl;
		}
		...
		if (pathVec.size() == 0) { // no valid path, maze is unsolvable
			...
			ofs << temp << "\n" << aptr->toString() << std::endl << is_valid_path(pathVec, aptr->toString()) << ":\n" << ss.str() << std::endl;
		}
	}
...
std::string is_valid_path(vector<string> &vec, std::string mazeString) {
	...
	stringstream pathSS("Path Given:\n"); // will hold a string representation of the path given in vec
	...
	//	cout << "START START START" << vec[6] << endl;
	...
	for (int i = 0; i < vec.size(); i++) { // for each of the coordinates given:
		...
		(xloc == newXLoc) ? xMov = 0 : xMov++; // if x has changed, increment xMov
		(yloc == newYLoc) ? yMov = 0 : yMov++; // if y has changed, increment yMov
		(zloc == newZLoc) ? zMov = 0 : zMov++; // if z has changed, increment zMov
